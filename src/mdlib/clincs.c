...
static void do_lincs(rvec *x,rvec *xp,matrix box,t_pbc *pbc,
                     ...
                     real invdt,rvec *v,
                     gmx_bool bCalcVir,tensor rmdr, gmxLS_locals_grid_t *locals_grid)
{
    int     b,i,j,k,n,iter,ii,jj;
    ...
    int     *nlocat;
    real     fx,fy,fz,ccc, dax, day, daz, dbx, dby, dbz, gix, giy, giz, gjx, gjy, gjz;
    rvec    *xuncon;
    ...
    *warn = 0;
    ...
    snew(xuncon,2*ncons);
    for(b=0; b<ncons; b++){
      copy_rvec(xp[bla[2*b]], xuncon[2*b]);
      copy_rvec(xp[bla[2*b+1]], xuncon[2*b+1]);
    }
    /*end JMV*/
    ...
    if (bCalcVir)
    {
        ...
        for(b=0; b<ncons; b++)
        {
            ...
            /* begin stress tensor */
            if ((locals_grid->contrib == enAll) || (locals_grid->contrib == enLincs))            
            {
...
gmx_bool constrain_lincs(FILE *fplog,gmx_bool bLog,gmx_bool bEner,
                     ...
                     int econq,
                     t_nrnb *nrnb,
                     int maxwarn,int *warncount,gmxLS_locals_grid_t *locals_grid)
{
    ...
    if (econq == econqCoord)
    {
        ...
        do_lincs(x,xprime,box,pbc_null,lincsd,md->invmass,cr,
                 ...
                 invdt,v,bCalcVir,rmdr,locals_grid);
