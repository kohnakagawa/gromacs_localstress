...
static void combine_forces(int nstlist,
                           ...
                           int start,int nrend,
                           rvec f[],rvec f_lr[],
                           t_nrnb *nrnb, gmxLS_locals_grid_t *locals_grid)
{
    int  i,d,nm1;
    ...
    if (constr != NULL && !(ir->eConstrAlg == econtSHAKE && ir->epc == epcNO))
    {
        ...
        constrain(NULL,FALSE,FALSE,constr,idef,ir,NULL,cr,step,0,md,
                  ...
                  NULL,NULL,nrnb,econqForce,ir->epc==epcMTTK,state->veta,state->veta,locals_grid);
    }
    ...
    if (bDoConstr) 
    {
        ...
        if (EI_VV(inputrec->eI) && bFirstHalf) 
        {
            constrain(NULL,bLog,bEner,constr,idef,
                      ...
                      inputrec->epc==epcMTTK,state->veta,vetanew,locals_grid);
        } 
        else 
        {
            constrain(NULL,bLog,bEner,constr,idef,
                      ...
                      inputrec->epc==epcMTTK,state->veta,state->veta,locals_grid);
        }
    ...
    if ((inputrec->eI == eiSD2) && !(bFirstHalf))
    {
        ...
        if (bDoConstr) 
        {
            ...
            constrain(NULL,bLog,bEner,constr,idef,
                      ...
                      state->x,xprime,NULL,
                      state->box,state->lambda,dvdlambda,
                      NULL,NULL,nrnb,econqCoord,FALSE,0,0,locals_grid);
            wallcycle_stop(wcycle,ewcCONSTR);
        }
    }    
                   ...
                   gmxLS_locals_grid_t *locals_grid)
{
    ...
    if (bDoLR && inputrec->nstlist > 1 && !EI_VV(inputrec->eI))  /* get this working with VV? */
    {
        ...
        combine_forces(inputrec->nstlist,constr,inputrec,md,idef,cr,step,state,
                       start,nrend,f,f_lr,nrnb,locals_grid);
        force = f_lr;
    }
    else
    {
