        ...
        gmxLS_locals_grid_t *locals_grid)
{
  ...
  {
      init_neighbor_list(fp, fr, md->homenr,locals_grid);
  }
...
#define TAKETIME FALSE
    ...
    do_nonbonded(cr,fr,x,f,md,excl,
                 ...
                 lambda,&dvdlambda,-1,-1,locals_grid,donb_flags);
    ...
    if (ir->n_flambda > 0 && (flags & GMX_FORCE_DHDL) && ir->sc_alpha != 0)
    {
        ...
        for(i=0; i<enerd->n_lambda; i++)
        {
            ...
            do_nonbonded(cr,fr,x,f,md,excl,
                         ...
                         enerd->grpp.ener[egGB], box_size,nrnb,
                         lam_i,&dvdl_dum,-1,-1,locals_grid,
    ...
    if (flags & GMX_FORCE_BONDED)
    {
        ...
        calc_bonds(fplog,cr->ms,
                   ...
                   fr->bSepDVDL && do_per_step(step,ir->nstlog),step,locals_grid);
            ...
            idef->ilsort != ilsortNO_FE)
        {
            ...
            for(i=0; i<enerd->n_lambda; i++)
            {
                ...
                calc_bonds_lambda(fplog,
                                  ...
                                  fcd,
                                  DOMAINDECOMP(cr) ? cr->dd->gatindex : NULL,locals_grid);
                sum_epot(&ir->opts,&ed_lam);
                enerd->enerpart_lambda[i] += ed_lam.term[F_EPOT];
            }
            destroy_enerdata(&ed_lam);
            ...
            if (fr->eeltype != eelRF_NEC)
            {
                enerd->term[F_RF_EXCL] =
                    RF_excl_correction(fplog,fr,graph,md,excl,x,f,
                                       fr->fshift,&pbc,lambda,&dvdlambda,locals_grid);
            }
