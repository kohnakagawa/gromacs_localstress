...
void locals_initialize(gmxLS_locals_grid_t *locals_grid )
{
    int i,j,k;
    int ngrid;
    ...
    if ( locals_grid->spatatom == enSpat )
    {
        ...
        for(i=0;i<ngrid;i++)
        {
            for(j=0;j<DIM;j++)
                for(k=0;k<DIM;k++)
                    locals_grid->sum_grid[i][j][k] =0;
        }
        
    }
    else if ( locals_grid->spatatom == enAtom )
    {
    ...
    locals_grid->nframes = 0;
}

int write_locals_grid(int nfile,const t_filenm fnm[],gmxLS_locals_grid_t *locals_grid, t_state *state,int frameindex)
{
    int i,j,k,ngrid;
    char buf[1024];
    FILE *fplocal;
    ...
    snprintf(buf,1024,"%s%d",opt2fn("-ols",nfile,fnm),frameindex);
    
    printf("\n\nDumping local stress based on %d frames to %s...\n", locals_grid->nframes, buf);
    
    fplocal=fopen(buf,"w"); /* localstress.dat by default */

    if ( locals_grid->spatatom == enSpat )
    {

        msmul(locals_grid->avgbox, 1.0/locals_grid->nframes, locals_grid->avgbox);
        i=(sizeof(real)==sizeof(double)) ? 1 : 0;
        fwrite(&i,sizeof(int),1,fplocal); /* 1 if double, 0 if single precision */
        fwrite(&locals_grid->avgbox,sizeof(real),9,fplocal);
        fwrite(&locals_grid->nx,sizeof(int),1,fplocal);
        fwrite(&locals_grid->ny,sizeof(int),1,fplocal);
        fwrite(&locals_grid->nz,sizeof(int),1,fplocal);
        ngrid=locals_grid->nx*locals_grid->ny*locals_grid->nz;
        ccc = PRESFAC/locals_grid->nframes;

        for(i=0;i<ngrid;i++)
        {
            for(j=0;j<DIM;j++)
                for(k=0;k<DIM;k++)
                    locals_grid->sum_grid[i][j][k]*=ccc;
        }
    }
    else if ( locals_grid->spatatom == enAtom )
    {
        ...
        fwrite(&i,sizeof(int),1,fplocal); /* 1 if double, 0 if single precision */
        fwrite(&locals_grid->avgbox,sizeof(real),9,fplocal);
        fwrite(&ngrid,sizeof(int),1,fplocal);
        ccc = PRESFAC/locals_grid->nframes;
        for(i=0;i<ngrid;i++)
        {
            for(j=0;j<DIM;j++)
    ...
    fwrite(locals_grid->sum_grid,sizeof(matrix),ngrid,fplocal);
