...
void csettle(gmx_settledata_t settled,
             ...
             real invdt,real *v,gmx_bool bCalcVir,tensor rmdr,int *error,t_vetavars *vetavar,gmxLS_locals_grid_t *locals_grid)
{
    /* ***************************************************************** */
    /*                                                               ** */
    ...
    */
    ...
    if(!doshake) {
      ...
      int  lpatIDs[3];
          ...
          Ri[XX] = b4[ow1]; Ri[YY] = b4[ow1+1]; Ri[ZZ] = b4[ow1+2];
          Rj[XX] = b4[hw2]; Rj[YY] = b4[hw2+1]; Rj[ZZ] = b4[hw2+2];
          Rk[XX] = b4[hw3]; Rk[YY] = b4[hw3+1]; Rk[ZZ] = b4[hw3+2];
          Fi[XX] = mdax; Fi[YY] = mday; Fi[ZZ] = mdaz;
          Fj[XX] = mdbx; Fj[YY] = mdby; Fj[ZZ] = mdbz;
          Fk[XX] = mdcx; Fk[YY] = mdcy; Fk[ZZ] = mdcz;
         ...
         if ((locals_grid->contrib == enAll) || (locals_grid->contrib == enSettle))
        {
            ...
            lpatIDs[0] = iatoms[i*2+1]; lpatIDs[1] = iatoms[i*2+1]+1; lpatIDs[2] = iatoms[i*2+1]+2;
