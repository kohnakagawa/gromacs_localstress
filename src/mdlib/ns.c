...
void init_neighbor_list(FILE *log,t_forcerec *fr,int homenr,gmxLS_locals_grid_t *locals_grid)
{
   ...
   fr->ns.bCGlist = (getenv("GMX_NBLISTCG") != 0) || locals_grid->CGlocals ;
   if (!fr->ns.bCGlist)
   {
...
static void do_longrange(t_commrec *cr,gmx_localtop_t *top,t_forcerec *fr,
                         ...
                         gmx_bool bHaveVdW[],
                         gmx_bool bDoForces,rvec *f,gmxLS_locals_grid_t *locals_grid)
{
    int n,i;
    t_nblist *nl;
        ...
        for(i=0; (i<eNL_NR); i++)
        {
            ...
            if ((nl->nri > nl->maxnri-32) || bEvaluateNow)
            {
                ...
                do_nonbonded(cr,fr,x,f,md,NULL,
                             ...
                             nrnb,lambda,dvdlambda,n,i,locals_grid,
...
static int nsgrid_core(FILE *log,t_commrec *cr,t_forcerec *fr,
                       ...
                       gmx_bool bMakeQMMMnblist,gmxLS_locals_grid_t *locals_grid)
{
    gmx_ns_t *ns;
                    ...
                    for(nn=0; (nn<ngid); nn++)
                    {
                        ...
                        if (nlr_ljc[nn] > 0)
                        {
                            do_longrange(cr,top,fr,ngid,md,icg,nn,nlr_ljc[nn],
                                         ...
                                         put_in_list,bHaveVdW,bDoForces,f,locals_grid);
                        }
    ...
    for (nn=0; (nn<ngid); nn++)
    {
        if (rm2 > rs2)
        {
            do_longrange(cr,top,fr,0,md,icg,nn,nlr_ljc[nn],
                         nl_lr_ljc[nn],bexcl,shift,x,box_size,nrnb,
                         lambda,dvdlambda,grppener,
                         TRUE,TRUE,TRUE,put_in_list,bHaveVdW,bDoForces,f,locals_grid);
        }
        if (rl2 > rm2) {
            do_longrange(cr,top,fr,0,md,icg,nn,nlr_one[nn],
                         ...
                         TRUE,put_in_list,bHaveVdW,bDoForces,f,locals_grid);
        }
    }
    debug_gmx();
