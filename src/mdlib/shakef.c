...
int vec_shakef(FILE *fplog,gmx_shakedata_t shaked,
               ...
               t_vetavars *vetavar, gmxLS_locals_grid_t *locals_grid)
{
    rvec *rij;
    real *M2,*tt,*dist2;
    int     maxnit=1000;
    ...
    for(ll=0; (ll<ncon); ll++,ia+=3) 
    {
        ...
        if (bCalcVir)
        {
            ...
            /* begin stress tensor */
            ccc = lagr[ll]*invdt*invdt;
            fx = rij[ll][0]*ccc;
            fy = rij[ll][1]*ccc;
            fz = rij[ll][2]*ccc;
            
            /* begin stress tensor */
            if ((locals_grid->contrib == enAll) || (locals_grid->contrib == enCoul))
            {
             ...
             gmxLS_locals_grid_t *locals_grid)
{
  ...
  for(i=0; (i<nblocks); ) {
    ...
    n0 = vec_shakef(log,shaked,natoms,invmass,blen,idef->iparams,
                    ...
                    ir->efep!=efepNO,lambda,lam,invdt,v,bCalcVir,rmdr,econq,vetavar,locals_grid);
