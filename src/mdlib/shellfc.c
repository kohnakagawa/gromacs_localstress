...
static void init_adir(FILE *log,gmx_shellfc_t shfc,
		      ...
		      real lambda,real *dvdlambda,t_nrnb *nrnb,gmxLS_locals_grid_t *locals_grid)
{
  rvec   *xnold,*xnew;
  double w_dt;
  int    gf,ga,gt;
  ...
  constrain(log,FALSE,FALSE,constr,idef,ir,NULL,cr,step,0,md,
	    ...
	    lambda,dvdlambda,NULL,NULL,nrnb,econqCoord,FALSE,0,0,locals_grid);
  constrain(log,FALSE,FALSE,constr,idef,ir,NULL,cr,step,0,md,
  ...
  constrain(log,FALSE,FALSE,constr,idef,ir,NULL,cr,step,0,md,
	    ...
	    lambda,dvdlambda,NULL,NULL,nrnb,econqDeriv_FlexCon,FALSE,0,0,locals_grid); 
}
...
#define  Try (1-Min)             /* At start Try = 1 */
  ...
  do_force(fplog,cr,inputrec,mdstep,nrnb,wcycle,top,mtop,groups,
	   ...
	   state->lambda,graph,
	   fr,vsite,mu_tot,t,fp_field,NULL,bBornRadii,locals_grid,
  ...
  if (nflexcon) {
    init_adir(fplog,shfc,
	      ...
	      shfc->acc_dir-start,state->box,state->lambda,&dum,nrnb,locals_grid);
  ...
  for(count=1; (!(*bConverged) && (count < number_steps)); count++) {
    ...
    if (nflexcon) {
      init_adir(fplog,shfc,
		...
		state->box,state->lambda,&dum,nrnb,locals_grid);
