...
void do_force(FILE *fplog,t_commrec *cr,
              ...
              gmx_bool bBornRadii,gmxLS_locals_grid_t *locals_grid,
              int flags)
{
    int    cg0,cg1,i,j;
    int    start,homenr;
  ...
  if (!(cr->duty & DUTY_PME)) {
    ...
    if (bNS)
    {
        ...
        ns(fplog,fr,x,box,
           ...
           bDoLongRange,bDoForces,bSepLRF ? fr->f_twin : f,locals_grid);
        if (bSepDVDL)
        {
...
void do_constrain_first(FILE *fplog,gmx_constr_t constr,
                        ...
                        gmxLS_locals_grid_t *locals_grid)
{
    int    i,m,start,end;
    ...
    constrain(NULL,TRUE,FALSE,constr,&(top->idef),
              ...
              NULL,NULL,nrnb,econqCoord,ir->epc==epcMTTK,state->veta,state->veta,locals_grid);
    if (EI_VV(ir->eI)) 
    {
        ...
        constrain(NULL,TRUE,FALSE,constr,&(top->idef),
                  ...
                  state->box,state->lambda,&dvdlambda,
                  NULL,NULL,nrnb,econqVeloc,ir->epc==epcMTTK,state->veta,state->veta,locals_grid);
    }
     ...
     * as reference coordinates.                                                     
        ...
        constrain(NULL,TRUE,FALSE,constr,&(top->idef),
                  ...
                  state->x,savex,NULL,
                  state->box,state->lambda,&dvdlambda,
                  state->v,NULL,nrnb,econqCoord,ir->epc==epcMTTK,state->veta,state->veta,locals_grid);
    ...
    *dvdlcorr = 0;
    ...
    if (ir->eDispCorr != edispcNO) {
        ...
        *enercorr += avcsix*enerdiff;
        ...
        {
            for(m=0; m<DIM; m++)
                locals_grid->current_grid[i][m][m] -= svir/ngrid;
        }
        */
